...
namespace Shopping.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Index(string sortOrder, string currentFilter, string searchString, int? pageNumber)
        {
            ...
            {
                Products = await PaginatedList<Product>.CreateAsync(query, pageNumber ?? 1, pageSize), //?? Doble validacion de nulos, si viene nulo le asigna 1 por defecto
        ...
        public async Task<IActionResult> Add(int? id)//Agregar producto al carrito
        {
            if (id == null)
            {
                return NotFound();
            ...
            if (!User.Identity.IsAuthenticated)//si el usuario no está autenticado lo mandamos al login
            {
